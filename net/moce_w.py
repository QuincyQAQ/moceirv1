...
class PSFPyramid(nn.Module):
    def __init__(self, in_ch, dims):
        super().__init__()
        self.proj0 = nn.Conv2d(in_ch, dims[0], 3, padding=1)
        self.projs = nn.ModuleList(
            [
                nn.Sequential(nn.AvgPool2d(2), nn.Conv2d(dims[i], dims[i + 1], 3, padding=1))
                for i in range(len(dims) - 1)
            ]
        )

    def forward(self, psf, target_hw_list):
        (h0, w0) = target_hw_list[0]
        psf0 = F.interpolate(psf, size=(h0, w0), mode="bilinear", align_corners=False)
        out = []
        x = self.proj0(psf0)
        out.append(x)
        for i, blk in enumerate(self.projs):
            x = blk(x)
            hi, wi = target_hw_list[i + 1]
            if x.shape[-2:] != (hi, wi):
...
class MoCEIR(nn.Module):
    def __init__(self,
        ...
        self.psf = nn.Parameter(torch.randn(1, inp_channels, 256, 256))
        self.w = nn.Parameter(torch.tensor([0.001], dtype=torch.float32))
        self.delta = nn.Parameter(torch.tensor(np.ones(levels-1) * 0.01, dtype=torch.float32))
        self.psf_pyr = PSFPyramid(inp_channels, dims[:-1])
    ...
    def forward(self, x, labels=None):
        ...
        target_hw_list = []
        tmp = feats
        for (block, downsample) in self.enc:
            target_hw_list.append(tmp.shape[-2:])
            tmp = downsample(tmp)
        psf_list = self.psf_pyr(self.w * self.psf, target_hw_list)

        enc_feats = []
        for i, (block, downsample) in enumerate(self.enc):
            feats = block(feats)
            feats = WieNer(feats, psf_list[i], self.delta[i])
