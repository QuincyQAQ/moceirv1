...
def generate_coord_features(h, w, device):
    ...
    r_map = torch.sqrt(x_coords ** 2 + y_coords ** 2)
    r_map = r_map / (r_map.max() + 1e-6) * 2 - 1
    return torch.cat([x_coords, y_coords, r_map], dim=1)
...
class SpectralExpert(nn.Module):
    def __init__(self, dim, kernel_size=None, patch_size=None):
        super().__init__()
        self.dim = dim
        self.spectral_filter = nn.Conv2d(dim * 2, dim * 2, kernel_size=1, bias=False)

    def forward(self, x):
        in_dtype = x.dtype
        fft_x = torch.fft.rfft2(x.float(), dim=(-2, -1), norm="ortho")
        fft_x_split = torch.cat([fft_x.real, fft_x.imag], dim=1)
        processed_fft_split = self.spectral_filter(fft_x_split)
        processed_real, processed_imag = torch.chunk(processed_fft_split, 2, dim=1)
        processed_fft = torch.complex(processed_real, processed_imag)
        out = torch.fft.irfft2(processed_fft, s=x.shape[-2:], dim=(-2, -1), norm="ortho")
...
class ModExpert(nn.Module):
    def __init__(self, dim: int, rank: int, func: nn.Module, depth: int, patch_size: int, kernel_size:int):
        ...
        self.depth = depth
        self.func = func
        self.is_spectral = (func == SpectralExpert)

        if self.is_spectral:
            self.proj = None
            self.body = SpectralExpert(dim)
        else:
...
class AdapterLayer(nn.Module):
    def __init__(self, 
                 ...
                 spectral_expert_indices: Optional[List[int]] = None):
        super().__init__()            
        ...
        if spectral_expert_indices is None:
            spectral_expert_indices = [num_experts - 1]
        spectral_expert_indices = set(int(i) for i in spectral_expert_indices)
        ...
        self.experts = nn.ModuleList([
            MySequential(*[
                ModExpert(
                    dim,
                    rank=rank,
                    func=(SpectralExpert if idx in spectral_expert_indices else expert_layer),
                    depth=depth,
                    patch_size=patch,
                    kernel_size=kernel,
                )
...
class RoutingFunction(nn.Module):
    ...
    def forward(self, x, freq_emb):
        h, w = x.shape[-2:]
        coord_features = generate_coord_features(h, w, x.device).expand(x.shape[0], -1, -1, -1)
        coord_features = coord_features.to(dtype=x.dtype)
        x_with_coords = torch.cat([x, coord_features], dim=1)
        logits = self.gate(x_with_coords) + self.freq_gate(freq_emb)
        if self.training:
