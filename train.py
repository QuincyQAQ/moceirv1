...
class PLTrainModel(pl.LightningModule):
    def __init__(self, opt):
        ...
        module = importlib.import_module(f"net.{self.model_name}")
        build_fn = getattr(module, "build_model", None)
        if build_fn is None:
            raise AttributeError(f"net.{self.model_name} must define build_model(opt)")
        self.net = build_fn(opt)
        self.uses_balance_loss = hasattr(self.net, "total_loss")
    ...
    def training_step(self, batch, batch_idx):
        ...
        restored = _forward_model(self.net, degrad_patch, de_id)
        balance_loss = _extract_balance_loss(self.net, restored)
...
def main(opt):
    ...
    if run_id is None:
        ...
        # Keep run directory name filesystem-safe
