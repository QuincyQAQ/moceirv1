...
from net.acformer import ACFormer
...
class PLTrainModel(pl.LightningModule):
    def __init__(self, opt):
        ...
        # 根据模型类型构建网络
        if self.model_name in ("MoCE_IR_S", "MoCE_IR"):
            ...
            self.uses_balance_loss = True
        elif self.model_name == "ACFormer":
            # 使用 Metalens-Transformer 的 ACFormer 作为去畸变网络
            self.net = ACFormer(
                inp_channels=3,
                out_channels=3,
                dim=opt.ac_dim,
                num_blocks=opt.ac_num_blocks,
                num_refinement_blocks=opt.ac_num_refinement_blocks,
                channel_heads=opt.ac_channel_heads,
                spatial_heads=opt.ac_spatial_heads,
                overlap_ratio=opt.ac_overlap_ratio,
                window_size=opt.ac_window_size,
                spatial_dim_head=opt.ac_spatial_dim_head,
                bias=opt.ac_bias,
                ffn_expansion_factor=opt.ac_ffn_expansion_factor,
                layernorm_type=opt.ac_layernorm_type,
                M=opt.ac_M,
                ca_heads=opt.ac_ca_heads,
                ca_dim=opt.ac_ca_dim,
                window_size_ca=opt.ac_window_size_ca,
                query_ksize=opt.ac_query_ksize,
            )
            # 如需关闭角向 CA 模块，可在 config.ACFORMER_CONFIG 中将 use_ca 设为 False
            if not getattr(opt, "ac_use_ca", True):
                self.net.ca = None
            self.uses_balance_loss = False
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        else:  # ACFormer 不需要退化 ID，也没有内部 balance loss
            restored = self.net(degrad_patch)
            balance_loss = restored.new_zeros(1)
...
def main(opt):
    ...
    # only creates a single experiment/<model>-<timestamp> directory.
    run_id = os.environ.get("MOCEIRV2_RUN_ID")
    if run_id is None:
        timestamp = datetime.now().strftime('%Y_%m_%d_%H_%M_%S')
        # 构造一个简单的模型标识，便于从文件夹名直接看出本次使用的网络
        model_tag = opt.model
        if opt.model in ("MoCE_IR", "MoCE_IR_S"):
            backbone = getattr(opt, "moce_backbone", "moce_ir")
            model_tag = f"{opt.model}-{backbone}"
        run_id = f"{model_tag}-{timestamp}"
    ...
    # Per-run root directory under experiment/<model>-<timestamp>
    log_dir = base_exp_dir / time_stamp
    log_dir.mkdir(parents=True, exist_ok=True)

    # 保存当前网络代码快照，避免后续修改 network.py 后无法追溯本次实验的实现版本
    try:
        code_dir = log_dir / "net_snapshot"
        code_dir.mkdir(parents=True, exist_ok=True)
        net_dir = project_dir / "net"
        for fname in ["moce_ir.py", "moce_w.py", "acformer.py"]:
            src = net_dir / fname
            if src.exists():
                dst = code_dir / fname
                shutil.copy2(src, dst)
    except Exception as e:
        print(f"[Warn] Failed to snapshot network code: {e}")
